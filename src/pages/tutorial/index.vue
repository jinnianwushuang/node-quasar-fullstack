...
import { columns } from "src/pages/tutorial/config/index.js";
import { api_tutorial } from "src/api/index.js";
...
export default {
  ...
  methods: {
    init_table_data() {
      ...
      console.log("api_tutorial", api_tutorial);
      api_tutorial.get_tutorial_findAll(params).then(res => {
    ...
    handle_click_delete(item) {
      ...
      api_tutorial.delete_tutorial_by_id({ id: item.id }).then(res => {
        this.init_table_data();
      });
    },
    // 一键删除
    handle_click_delete_all() {
      console.log("删除所有");
      api_tutorial.delete_tutorial_all().then(res => {
        this.init_table_data();
      });
    },
    // 刷新
    ...
    handle_click_submit_add(obj) {
      ...
      api_tutorial.post_tutorial_create(params).then(res => {
        if (!obj) {
          this.init_table_data();
        }
      });
    ...
    handle_click_submit_edit() {
      let params = obj || this.editing_obj;
      api_tutorial.put_tutorial_by_id(params).then(res => {
        this.init_table_data();
      });
    }
  }
